...
def save_config():
    config = {"property_price": property_price,"current_home_value": current_home_value,"other_upfront_costs": other_upfront_costs,"use_equity": use_equity,"equity_percentage": equity_percentage if use_equity else None,"deposit_amount": additional_deposit if use_equity else None,"deposit_percentage": deposit_percentage_input if not use_equity else None,"capitalize_lmi": capitalize_lmi if initial_lvr > 80 else False,"loan_amount": loan_amount_final,"interest_rate": interest_rate,"loan_term": loan_term,
              # Updated income keys
              "user_fortnightly_income": user_fortnightly_income,"partner_fortnightly_income": partner_fortnightly_income,
              "include_rental": include_rental,"weekly_rental": weekly_rental if include_rental else 0,"occupancy_rate": occupancy_rate if include_rental else 0,"include_agistment": include_agistment,"num_cattle": num_cattle if include_agistment else 0,"agistment_rate": agistment_rate if include_agistment else 0,"fortnightly_living_expenses": fortnightly_living_expenses,"num_children_boarding": num_children_boarding,"annual_boarding_fee_per_child": annual_boarding_fee_per_child,"annual_council_rates": annual_council_rates,"annual_insurance": annual_insurance,"annual_maintenance": annual_maintenance,"annual_agistment_costs": annual_agistment_costs if include_agistment else 0,"annual_additional_property_expenses": annual_additional_property_expenses,
              # Future Events keys
              "include_super_events": include_super_events,"user_retire_year": user_retire_year,"user_super_amount": user_super_amount,"wife_retire_year": wife_retire_year,"wife_super_amount": wife_super_amount,"use_your_super_payoff": use_your_super_payoff,"post_retirement_income_user": post_retirement_income_user, "post_retirement_income_partner": post_retirement_income_partner, # Added partner post-ret income
              "include_education_change": include_education_change,"years_until_edu_change": years_until_edu_change,"new_annual_edu_cost_per_child": new_annual_edu_cost_per_child,"duration_of_new_cost": duration_of_new_cost,
              # Advanced Settings keys
              "inflation_rate": inflation_rate,"property_growth_rate": property_growth_rate,"income_growth_rate": income_growth_rate,"rental_growth_rate": rental_growth_rate,"projection_years": projection_years,"risk_analysis": risk_analysis}
    # Remove old combined income key if present from older saves (optional robustness)
    # config.pop("combined_fortnightly_income", None)
    return json.dumps(config, indent=4)
def load_config(config_data):
    # Add default values for newly added keys if they are missing in the loaded file
    config_data.setdefault('user_fortnightly_income', 2500)
    config_data.setdefault('partner_fortnightly_income', 2500)
    config_data.setdefault('post_retirement_income_user', 50000)
    config_data.setdefault('post_retirement_income_partner', 0)
    config_data.setdefault('use_your_super_payoff', True if config_data.get('include_super_events') else False) # Sensible default for payoff
    # Remove old key if present to avoid conflict
...
with col1_config: config_json_str = save_config(); st.download_button(label="Save Current Configuration (.json)", data=config_json_str, file_name="dorrigo_property_config.json", mime="application/json")
with col2_config:
    uploaded_file = st.file_uploader("Load Configuration (.json)", type=["json"])
    if uploaded_file is not None:
        try:
            config_data = json.load(uploaded_file)
            if st.button("Apply Loaded Configuration"): load_config(config_data); st.experimental_rerun()
        except json.JSONDecodeError: st.error("Invalid JSON file.")
        except Exception as e: st.error(f"Failed to load configuration: {e}")

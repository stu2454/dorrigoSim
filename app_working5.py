# --- START OF FILE app.py (Revised Load Logic with Callback) ---
...
def calculate_monthly_mortgage_payment(loan_amount, annual_interest_rate, loan_term_years):
    ...
    try:
        ...
        if denominator == 0: return loan_amount / num_payments
...
def calculate_loan_balance_over_time(loan_amount, annual_interest_rate, monthly_payment, loan_term_years):
    ...
    loan_balance = [loan_amount]; current_balance = loan_amount
    if monthly_payment <= 0:
        if annual_interest_rate > 0:
             ...
             for _ in range(num_payments): current_balance *= (1 + monthly_rate); loan_balance.append(current_balance)
             return loan_balance
        else: return [loan_amount] * (num_payments + 1)
    monthly_interest_rate = annual_interest_rate / 100 / 12
    for i in range(1, num_payments + 1):
        interest = max(0, current_balance * monthly_interest_rate); principal_paid = max(0, monthly_payment - interest)
        current_balance -= principal_paid; current_balance = max(0, current_balance)
        loan_balance.append(current_balance)
        if current_balance == 0: loan_balance.extend([0] * (num_payments - i)); break
    return loan_balance
    ...
    for year in range(num_years_df):
        ...
        if year_start_month_index < full_term_payments:
            ...
            for month_idx in range(year_start_month_index, min(year_end_month_index, full_term_payments)):
                 balance_before_payment = loan_balance[month_idx]; interest_this_month = max(0, balance_before_payment * monthly_rate); interest_paid_this_year += interest_this_month
            ...
            principal_paid_this_year = max(0, balance_at_start_of_year - balance_at_end_of_year); payment_this_year = principal_paid_this_year + interest_paid_this_year
...
def project_income_expenses(
    ...
    edu_change_ends_after_year = math.floor(years_until_edu_change); new_edu_cost_start_year = edu_change_ends_after_year + 1
    new_edu_cost_end_year = new_edu_cost_start_year + duration_of_new_cost - 1
    projected_employment = []; projected_rental = []; projected_agistment = []; projected_lump_sums = []
    projected_living = []; projected_education = []; projected_rates = []; projected_insurance = []
    projected_maintenance = []; projected_agistment_costs = []; projected_additional_prop = []
    for year in years:
        emp_growth = (1 + income_growth_rate / 100) ** year; rent_growth = (1 + rental_growth_rate / 100) ** year; inf_growth = (1 + inflation_rate / 100) ** year
        current_year_user_income = annual_user_income * emp_growth; current_year_partner_income = annual_partner_income * emp_growth
        current_year_rental = annual_rental_income * rent_growth; current_year_agistment = annual_agistment_income * inf_growth; current_year_lump_sum = 0
        ...
        current_year_living = annual_living_expenses * inf_growth; current_year_rates = annual_council_rates * inf_growth; current_year_insurance = annual_insurance * inf_growth; current_year_maintenance = annual_maintenance * inf_growth; current_year_agistment_costs = annual_agistment_costs * inf_growth; current_year_additional_prop = annual_additional_property_expenses * inf_growth
        current_year_education = 0; base_cost_no_inflation = annual_boarding_expenses; new_cost_base_no_inflation = new_annual_edu_cost_per_child * num_children_boarding
        ...
        projected_employment.append(current_year_employment); projected_rental.append(current_year_rental); projected_agistment.append(current_year_agistment); projected_lump_sums.append(current_year_lump_sum); projected_living.append(current_year_living); projected_education.append(current_year_education); projected_rates.append(current_year_rates); projected_insurance.append(current_year_insurance); projected_maintenance.append(current_year_maintenance); projected_agistment_costs.append(current_year_agistment_costs); projected_additional_prop.append(current_year_additional_prop)
    df['Employment_Income'] = projected_employment; df['Rental_Income'] = projected_rental; df['Agistment_Income'] = projected_agistment; df['Lump_Sum_Income'] = projected_lump_sums
    df['Total_Income'] = df['Employment_Income'] + df['Rental_Income'] + df['Agistment_Income'] + df['Lump_Sum_Income']
    df['Living_Expenses'] = projected_living; df['Education_Expenses'] = projected_education; df['Council_Rates'] = projected_rates; df['Insurance'] = projected_insurance; df['Maintenance'] = projected_maintenance; df['Agistment_Costs'] = projected_agistment_costs; df['Additional_Property_Expenses'] = projected_additional_prop
    ...
    df['Total_Expenses_Excl_Mortgage'] = (df['Living_Expenses'] + df['Education_Expenses'] + df['Council_Rates'] + df['Insurance'] + df['Maintenance'] + df['Agistment_Costs'] + df['Additional_Property_Expenses'])
...
# --- Sidebar Inputs ---
try: st.sidebar.image("dorrigo.jpg", caption="Dorrigo, NSW")
except FileNotFoundError: st.sidebar.warning("dorrigo.jpg not found."); st.sidebar.image("https://via.placeholder.com/300x100?text=Dorrigo+NSW")
...
with st.sidebar.expander("Loan & Deposit Details", expanded=True):
    ...
    if use_equity:
        ...
        total_available_deposit_funds = equity_amount + additional_deposit; st.info(f"Available Deposit Funds: ${total_available_deposit_funds:,.2f}\n(${equity_amount:,.2f} equity + ${additional_deposit:,.2f} cash)")
    else:
        deposit_percentage_input = st.slider("Deposit Percentage (%)", 10, 100, 20, 5, key="deposit_percentage")
        total_available_deposit_funds = property_price * (deposit_percentage_input / 100); st.info(f"Available Deposit Funds: ${total_available_deposit_funds:,.2f}")
    base_loan_amount = max(0, property_price - total_available_deposit_funds); estimated_lmi, initial_lvr = estimate_lmi(property_price, base_loan_amount); st.write(f"Initial Loan-to-Value Ratio (LVR): {initial_lvr:.2f}%"); lmi_payable = 0
    if initial_lvr > 80:
        ...
        if capitalize_lmi: loan_amount_final = base_loan_amount + estimated_lmi; lmi_payable = 0; st.info(f"LMI Capitalized. Final Loan Amount: ${loan_amount_final:,.0f}")
    ...
    else: st.success("LVR <= 80%. No LMI typically required."); loan_amount_final = base_loan_amount; st.session_state.capitalize_lmi = False; capitalize_lmi = False
    loan_amount_final = st.number_input("Final Loan Amount (AUD$) *", 0, int(property_price * 1.1), int(loan_amount_final), 1000, format="%d", key="loan_amount", help="Auto-calculated. You can override.")
    _, final_lvr = estimate_lmi(property_price, loan_amount_final); interest_rate = st.slider("Interest Rate (%)", 3.0, 10.0, 6.0, 0.1, key="interest_rate"); loan_term = st.slider("Loan Term (Years)", 10, 30, 25, 1, key="loan_term")
st.sidebar.markdown("## Income Sources (Annualized)")
with st.sidebar.expander("Your Employment Income", expanded=True): user_fortnightly_income = st.number_input("Your Fortnightly After-Tax Income (AUD$)", 0, 15000, 2500, 100, format="%d", key="user_fortnightly_income"); annual_user_income = user_fortnightly_income * 26; st.write(f"Annual: ${annual_user_income:,.2f}")
with st.sidebar.expander("Partner's Employment Income", expanded=True): partner_fortnightly_income = st.number_input("Partner's Fortnightly After-Tax Income (AUD$)", 0, 15000, 2500, 100, format="%d", key="partner_fortnightly_income"); annual_partner_income = partner_fortnightly_income * 26; st.write(f"Annual: ${annual_partner_income:,.2f}")
with st.sidebar.expander("Rental Income", expanded=False):
    ...
    if include_rental: weekly_rental = st.slider("Weekly Rental Income (AUD$)", 0, 1000, 450, 10, format="%d", key="weekly_rental"); occupancy_rate = st.slider("Occupancy Rate (%)", 0, 100, 90, 5, key="occupancy_rate"); annual_rental_income = weekly_rental * 52 * (occupancy_rate / 100); st.write(f"Annual: ${annual_rental_income:,.2f}")
...
with st.sidebar.expander("Agistment Income", expanded=False):
    ...
    if include_agistment: num_cattle = st.slider("Number of Cattle for Agistment", 0, 100, 20, 5, key="num_cattle"); agistment_rate = st.slider("Agistment Rate (AUD$/head/week)", 0.0, 20.0, 8.0, 0.5, key="agistment_rate"); annual_agistment_income = num_cattle * agistment_rate * 52; st.write(f"Annual: ${annual_agistment_income:,.2f}")
...
with st.sidebar.expander("Living & Education Expenses", expanded=True):
    fortnightly_living_expenses = st.number_input("Fortnightly Living Expenses (AUD$)", 0, 10000, 3000, 100, format="%d", key="fortnightly_living_expenses"); annual_living_expenses = fortnightly_living_expenses * 26; st.write(f"Annual Living: ${annual_living_expenses:,.2f}")
    num_children_boarding = st.number_input("Number of Children (for Education Costs)", 0, 10, 1, 1, key="num_children_boarding"); annual_boarding_fee_per_child = st.number_input("Initial Annual Education Fee per Child (AUD$)", 0, 100000, 50000, 1000, format="%d", key="annual_boarding_fee_per_child", help="e.g., Boarding school fee"); annual_boarding_expenses = num_children_boarding * annual_boarding_fee_per_child; st.write(f"Initial Annual Education: ${annual_boarding_expenses:,.2f}")
with st.sidebar.expander("Property Running Costs", expanded=True):
    annual_council_rates = st.number_input("Annual Council Rates (AUD$)", 0, 10000, 2500, 100, format="%d", key="annual_council_rates"); annual_insurance = st.number_input("Annual Insurance (Building/Contents/Liability) (AUD$)", 0, 10000, 2000, 100, format="%d", key="annual_insurance"); annual_maintenance = st.number_input("Annual Property Maintenance (AUD$)", 0, 30000, 6000, 500, format="%d", key="annual_maintenance")
    if include_agistment: annual_agistment_costs = st.number_input("Annual Agistment Costs (Fencing, Water etc.) (AUD$)", 0, 20000, 2000, 500, format="%d", key="annual_agistment_costs")
...
with st.sidebar.expander("Retirement & Super Access", expanded=False):
    ...
    if include_super_events:
        user_retire_year = st.number_input("Your Retirement Year (Years from now)", 0, proj_years_default, 7, 1, key="user_retire_year"); user_super_amount = st.number_input("Your Est. Super Access Amount (AUD$)", 0, 2000000, 700000, 10000, format="%d", key="user_super_amount"); post_retirement_income_user = st.number_input("Your Est. Post-Retirement Annual Income (AUD$)", 0, 200000, 50000, 1000, format="%d", key="post_retirement_income_user", help="Assumed constant (not inflated)."); st.markdown("---")
        wife_retire_year = st.number_input("Partner's Retirement Year (Years from now)", 0, proj_years_default, 10, 1, key="wife_retire_year"); wife_super_amount = st.number_input("Partner's Est. Super Access Amount (AUD$)", 0, 2000000, 600000, 10000, format="%d", key="wife_super_amount"); post_retirement_income_partner = st.number_input("Partner's Est. Post-Retirement Annual Income (AUD$)", 0, 200000, 0, 1000, format="%d", key="post_retirement_income_partner", help="Assumed constant."); st.markdown("---")
        use_your_super_payoff = st.checkbox("Use Your Super Access Amount to Pay Off Mortgage?", value=True, key="use_your_super_payoff")
    else: user_retire_year = proj_years_default + 1; user_super_amount = 0; post_retirement_income_user = 0; wife_retire_year = proj_years_default + 1; wife_super_amount = 0; post_retirement_income_partner = 0; use_your_super_payoff = False
with st.sidebar.expander("Future Education Costs", expanded=False):
    ...
    if include_education_change:
        st.info("Current 'Initial Education Fee' applies until the change point."); years_until_edu_change = st.number_input("Years Until Education Cost Change", 0.0, float(proj_years_default), 3.5, 0.5, key="years_until_edu_change"); new_annual_edu_cost_per_child = st.number_input("New Annual Education Cost per Child After Change (AUD$)", 0, 100000, 0, 1000, format="%d", key="new_annual_edu_cost_per_child", help="Enter 0 if support stops."); duration_of_new_cost = st.number_input("Duration of New Education Cost (Years)", 0, proj_years_default, 4, 1, key="duration_of_new_cost", help="How many years does the new cost apply?")
    else: years_until_edu_change = proj_years_default + 1; new_annual_edu_cost_per_child = 0; duration_of_new_cost = 0
with st.sidebar.expander("Advanced Settings & Projections", expanded=False):
    inflation_rate = st.slider("Annual Inflation Rate (Expenses Growth) (%)", 0.0, 10.0, 2.5, 0.1, key="inflation_rate"); property_growth_rate = st.slider("Annual Property Value Growth Rate (%)", -5.0, 10.0, 4.0, 0.1, key="property_growth_rate"); income_growth_rate = st.slider("Annual Employment Income Growth Rate (%)", 0.0, 10.0, 3.0, 0.1, key="income_growth_rate", help="Applies to pre-retirement salaries"); rental_growth_rate = st.slider("Annual Rental Income Growth Rate (%)", 0.0, 10.0, 3.5, 0.1, key="rental_growth_rate"); max_event_year = 0
    ...
    proj_years_default_final = max(loan_term, max_event_year, 10); projection_years = st.slider("Projection Years", 1, 40, int(proj_years_default_final), 1, key="projection_years"); risk_analysis = st.checkbox("Include Risk Analysis Section", value=True, key="risk_analysis")

# --- Main Calculation Section ---
stamp_duty = calculate_nsw_stamp_duty(property_price); actual_deposit_paid = max(0, property_price - loan_amount_final); funds_needed_for_deposit_and_costs = actual_deposit_paid + stamp_duty + lmi_payable + other_upfront_costs; monthly_mortgage_payment = calculate_monthly_mortgage_payment(loan_amount_final, interest_rate, loan_term); loan_balance = calculate_loan_balance_over_time(loan_amount_final, interest_rate, monthly_mortgage_payment, loan_term); loan_data = calculate_annual_totals(loan_balance, monthly_mortgage_payment, loan_term, projection_years, interest_rate)
projection_data = project_income_expenses(annual_user_income, annual_partner_income, annual_rental_income, annual_agistment_income, annual_living_expenses, annual_boarding_expenses, annual_council_rates, annual_insurance, annual_maintenance, annual_agistment_costs, annual_additional_property_expenses, loan_data, projection_years, income_growth_rate, inflation_rate, rental_growth_rate, num_children_boarding, include_super_events, user_retire_year, user_super_amount, wife_retire_year, wife_super_amount, post_retirement_income_user, post_retirement_income_partner, include_education_change, years_until_edu_change, new_annual_edu_cost_per_child, duration_of_new_cost)
...
if include_super_events and use_your_super_payoff:
    if user_retire_year <= projection_years and not merged_data[merged_data['Year'] == user_retire_year].empty:
        loan_balance_at_payoff_year = merged_data.loc[merged_data['Year'] == user_retire_year, 'Loan_Balance'].iloc[0]; super_amount_available_for_payoff = user_super_amount; amount_paid_off = max(0, min(super_amount_available_for_payoff, loan_balance_at_payoff_year)); new_loan_balance_after_payoff = max(0, loan_balance_at_payoff_year - amount_paid_off); super_residual = super_amount_available_for_payoff - amount_paid_off
        if amount_paid_off > 0:
            payoff_applied_info = (f"Mortgage payoff applied Year {user_retire_year}. Amt: ${amount_paid_off:,.0f}. Residual: ${super_residual:,.0f}."); st.sidebar.success(payoff_applied_info)
            payoff_mask = merged_data['Year'] >= user_retire_year; merged_data.loc[payoff_mask, 'Loan_Balance'] = new_loan_balance_after_payoff; merged_data.loc[payoff_mask, 'Annual_Mortgage_Payment'] = 0; merged_data.loc[merged_data['Year'] == user_retire_year, 'Mortgage_Lump_Sum_Payment'] = amount_paid_off; merged_data.loc[payoff_mask, 'Total_Expenses'] = merged_data['Total_Expenses_Excl_Mortgage']
            merged_data['Net_Cashflow'] = merged_data['Total_Income'] - merged_data['Total_Expenses'] - merged_data['Mortgage_Lump_Sum_Payment']; merged_data.loc[merged_data['Year'] == 0, 'Net_Cashflow'] = 0; merged_data['Cumulative_Cashflow'] = merged_data['Net_Cashflow'].cumsum()
merged_data['Property_Value'] = [property_price * ((1 + property_growth_rate/100) ** year) for year in merged_data['Year']]; merged_data['Equity'] = merged_data['Property_Value'] - merged_data['Loan_Balance']; merged_data['LVR_Percent'] = (merged_data['Loan_Balance'] / merged_data['Property_Value'].replace(0, np.nan)) * 100; merged_data['LVR_Percent'] = merged_data['LVR_Percent'].fillna(0).clip(lower=0)

# --- Main Content Area - Results ---
st.markdown("<div class='upfront-summary'>", unsafe_allow_html=True); st.markdown("### Upfront Funds Required & Summary"); col1_upfront, col2_upfront = st.columns(2)
...
st.markdown("<h2 class='sub-header'>Financial Summary (Year 1)</h2>", unsafe_allow_html=True); col1_yr1, col2_yr1, col3_yr1 = st.columns(3); idx_yr1 = 1 if len(merged_data) > 1 else 0; total_income_yr1 = merged_data.loc[idx_yr1, 'Total_Income']; combined_employment_yr1 = merged_data.loc[idx_yr1, 'Employment_Income']
with col1_yr1: st.markdown(f"### Total Annual Income (Yr 1)"); st.markdown(f"<h3 class='positive'>${total_income_yr1:,.2f}</h3>", unsafe_allow_html=True); st.markdown("---"); st.markdown(f"Employment (Combined): ${combined_employment_yr1:,.2f}");
...
total_expenses_yr1 = merged_data.loc[idx_yr1, 'Total_Expenses']; annual_mortgage_payment_yr1 = merged_data.loc[idx_yr1, 'Annual_Mortgage_Payment']; prop_running_costs_yr1 = merged_data.loc[idx_yr1, ['Council_Rates', 'Insurance', 'Maintenance', 'Agistment_Costs', 'Additional_Property_Expenses']].sum(); living_expenses_yr1 = merged_data.loc[idx_yr1, 'Living_Expenses']; education_expenses_yr1 = merged_data.loc[idx_yr1, 'Education_Expenses']
with col2_yr1: st.markdown(f"### Total Annual Expenses (Yr 1)"); st.markdown(f"<h3 class='negative'>${total_expenses_yr1:,.2f}</h3>", unsafe_allow_html=True); st.markdown("---"); st.markdown(f"Mortgage: ${annual_mortgage_payment_yr1:,.2f}"); st.markdown(f"Living: ${living_expenses_yr1:,.2f}"); st.markdown(f"Education: ${education_expenses_yr1:,.2f}"); st.markdown(f"Property Running: ${prop_running_costs_yr1:,.2f}")
...
st.markdown("<h2 class='sub-header'>Break-even & Key Ratios</h2>", unsafe_allow_html=True); break_even_year = None; cumulative_cashflow_positive_df = merged_data[merged_data['Year'] >= 1]; positive_cumulative_years = cumulative_cashflow_positive_df[cumulative_cashflow_positive_df['Cumulative_Cashflow'] >= 0]
...
with col2_be: debt_to_income_yr1 = loan_amount_final / total_income_yr1 if total_income_yr1 else 0; expense_to_income_yr1 = total_expenses_yr1 / total_income_yr1 if total_income_yr1 else 0; mortgage_to_income_yr1 = annual_mortgage_payment_yr1 / total_income_yr1 if total_income_yr1 else 0; st.markdown(f"**Initial LVR**: {final_lvr:.2f}%"); st.markdown(f"**Debt to Income (Yr 1)**: {debt_to_income_yr1:.2f}x"); st.markdown(f"**Expense to Income (Yr 1)**: {expense_to_income_yr1:.2f}x"); st.markdown(f"**Mortgage to Income (Yr 1)**: {mortgage_to_income_yr1:.2f}x")
st.markdown("<h2 class='sub-header'>Cash Flow Projection</h2>", unsafe_allow_html=True); fig_cashflow = go.Figure(); fig_cashflow.add_trace(go.Bar(x=merged_data['Year'], y=merged_data['Total_Income'], name='Total Income (incl. Lump Sums)', marker_color='#1E5631')); fig_cashflow.add_trace(go.Bar(x=merged_data['Year'], y=merged_data['Total_Expenses'], name='Total Expenses', marker_color='#CA3433')); fig_cashflow.add_trace(go.Scatter(x=merged_data['Year'], y=merged_data['Net_Cashflow'], name='Net Cashflow (Annual)', mode='lines+markers', line=dict(color='#365F91', width=3), marker=dict(size=6))); fig_cashflow.add_trace(go.Scatter(x=merged_data['Year'], y=merged_data['Cumulative_Cashflow'], name='Cumulative Cashflow', mode='lines', line=dict(color='#F49130', width=3, dash='dot')))
if include_super_events:
    if user_retire_year <= projection_years and user_retire_year >= 0: fig_cashflow.add_vline(x=user_retire_year, line_width=2, line_dash="dash", line_color="white", annotation_text="Your Ret.", annotation_position="top left", annotation_font_size=10, annotation_font_color="blue")
    if wife_retire_year <= projection_years and wife_retire_year >= 0: fig_cashflow.add_vline(x=wife_retire_year, line_width=2, line_dash="dash", line_color="white", annotation_text="Partner Ret.", annotation_position="top right", annotation_font_size=10, annotation_font_color="purple")
...
st.markdown("<h2 class='sub-header'>Income & Expense Breakdown (Year 1)</h2>", unsafe_allow_html=True); col1_pie, col2_pie = st.columns(2)
with col1_pie: income_data_yr1 = {'Category': ['Employment (Combined)', 'Rental', 'Agistment', 'Lump Sums'],'Amount': [merged_data.loc[idx_yr1, 'Employment_Income'],merged_data.loc[idx_yr1, 'Rental_Income'],merged_data.loc[idx_yr1, 'Agistment_Income'],merged_data.loc[idx_yr1, 'Lump_Sum_Income']]}; income_df_yr1 = pd.DataFrame(income_data_yr1); income_df_yr1 = income_df_yr1[income_df_yr1['Amount'] > 0.01];
...
with col2_pie: expense_data_yr1 = {'Category': ['Mortgage', 'Living', 'Education', 'Prop. Running Costs'],'Amount': [merged_data.loc[idx_yr1, 'Annual_Mortgage_Payment'],merged_data.loc[idx_yr1, 'Living_Expenses'],merged_data.loc[idx_yr1, 'Education_Expenses'],prop_running_costs_yr1]}; expense_df_yr1 = pd.DataFrame(expense_data_yr1); expense_df_yr1 = expense_df_yr1[expense_df_yr1['Amount'] > 0.01];
...
if risk_analysis:
    st.markdown("<h2 class='sub-header'>Risk Analysis (Year 1 Impact)</h2>", unsafe_allow_html=True); base_occupancy = occupancy_rate / 100 if include_rental else 0; base_num_cattle = num_cattle if include_agistment else 0; base_annual_rental_income_yr1 = merged_data.loc[idx_yr1, 'Rental_Income']; base_annual_agistment_income_yr1 = merged_data.loc[idx_yr1, 'Agistment_Income']; base_annual_employment_income_yr1 = merged_data.loc[idx_yr1, 'Employment_Income']; base_expenses_excl_mortgage_yr1 = living_expenses_yr1 + education_expenses_yr1 + prop_running_costs_yr1; risk_scenarios = {"Base Case": {"occupancy_factor": 1.0,"agistment_factor": 1.0,"interest_rate_increase": 0.0}, "No Rental Income": {"occupancy_factor": 0.0,"agistment_factor": 1.0,"interest_rate_increase": 0.0}, "No Agistment Income": {"occupancy_factor": 1.0,"agistment_factor": 0.0,"interest_rate_increase": 0.0}, "Reduced Agistment (50%)": {"occupancy_factor": 1.0,"agistment_factor": 0.5,"interest_rate_increase": 0.0}, "Interest Rate +2%": {"occupancy_factor": 1.0,"agistment_factor": 1.0,"interest_rate_increase": 2.0}, "Interest Rate +3%": {"occupancy_factor": 1.0,"agistment_factor": 1.0,"interest_rate_increase": 3.0}, "Stress Test (Vacant 50%, Low Agist 50%, Rate +2%)": {"occupancy_factor": 0.5,"agistment_factor": 0.5,"interest_rate_increase": 2.0}}; scenario_results = []
    for scenario_name, params in risk_scenarios.items(): scen_rental_income = base_annual_rental_income_yr1 * params["occupancy_factor"]; scen_agistment_income = base_annual_agistment_income_yr1 * params["agistment_factor"]; scen_total_income = base_annual_employment_income_yr1 + scen_rental_income + scen_agistment_income; scen_interest_rate = interest_rate + params["interest_rate_increase"]; scen_loan_amount = loan_amount_final; scen_monthly_payment = calculate_monthly_mortgage_payment(scen_loan_amount, scen_interest_rate, loan_term); scen_annual_mortgage = scen_monthly_payment * 12; scen_total_expenses = base_expenses_excl_mortgage_yr1 + scen_annual_mortgage; scen_net_position = scen_total_income - scen_total_expenses; scenario_results.append({"Scenario": scenario_name,"Annual Net Position (Year 1)": scen_net_position,"Monthly Net Position (Year 1)": scen_net_position / 12,"Interest Rate (%)": scen_interest_rate,"Annual Mortgage": scen_annual_mortgage})
...
with st.expander("Detailed Annual Projection Data"):
    display_data = merged_data.copy(); cols_to_display = ['Year', 'Total_Income', 'Lump_Sum_Income', 'Total_Expenses', 'Mortgage_Lump_Sum_Payment', 'Net_Cashflow', 'Cumulative_Cashflow', 'Loan_Balance', 'Property_Value', 'Equity', 'LVR_Percent', 'Annual_Mortgage_Payment', 'Employment_Income', 'Rental_Income', 'Agistment_Income', 'Living_Expenses', 'Education_Expenses', 'Council_Rates', 'Insurance', 'Maintenance', 'Agistment_Costs', 'Additional_Property_Expenses']; cols_to_display = [col for col in cols_to_display if col in display_data.columns]; display_data = display_data[cols_to_display]
    ...
    st.dataframe(display_data, use_container_width=True); csv = merged_data.to_csv(index=False); st.download_button(label="Download Full Projection Data as CSV", data=csv, file_name="dorrigo_property_projection_detailed.csv", mime="text/csv")
...
if pdf_buffer: st.download_button(label="Download Summary Report (PDF)", data=pdf_buffer, file_name="dorrigo_property_summary_report.pdf", mime="application/pdf", key="download_pdf_button") # Added key
...
def save_config():
    ...
    config = {}
    known_keys = [ # List all keys corresponding to user inputs
    ...
    for key in known_keys:
        config[key] = st.session_state.get(key) # Get value directly from session state
    # Optional: Add specific logic for conditional keys if needed, but get() handles non-existence
...
def prepare_config_data(config_data):
    ...
    # Ensure essential keys have a default if missing entirely from JSON
...
def apply_loaded_config(prepared_data):
    ...
    # st.success("Apply button clicked. Applying configuration to session state...") # Optional debug
    try:
        for key, value in prepared_data.items():
            # Only update state if value is not None, to avoid clearing existing state unnecessarily
            if value is not None:
                st.session_state[key] = value
            # If you want None in the JSON to reset a widget to its code default,
            # you might need to delete the key:
            # elif key in st.session_state:
            #     del st.session_state[key]
        # No st.experimental_rerun() here, Streamlit handles it after the callback
    except Exception as e:
         # If error happens during state update, show it
...
with col2_config:
    ...
    if uploaded_file is not None:
        try:
            ...
            prepared_data_for_callback = prepare_config_data(parsed_config_data) # Prepare data before button definition

            st.button(
                "Apply Loaded Configuration",
                key="apply_config_button",
                on_click=apply_loaded_config, # Pass the function reference
                args=(prepared_data_for_callback,) # Pass prepared data to the callback
            )
        except json.JSONDecodeError:
            st.error("Invalid JSON file.")
        except Exception as e:
            st.error(f"Failed to load or process configuration: {e}")
            import traceback
...
# --- END OF FILE app.py (Revised Load Logic with Callback) ---